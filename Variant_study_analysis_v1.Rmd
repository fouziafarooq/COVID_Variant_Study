...
```{r}
country_id <- country_id %>% rbind(c(16, "", "Switzerland", "Switzerland"))
country_id <- country_id %>% rbind(c(16, "", "France", "France"))
```

Creating maternal and fetal composite outcomes to aggregated swiss/french data:
```{r}
...
df4 <- df3 %>% group_by(country, week) %>% 
  ...
  dplyr::filter(country %in% c("Kenya",  "Spain", "Hong Kong", "Italy",
                               "South Africa", "USA",  "China","Chile", "Mexico", "Colombia", 
                               "USA", "India", "Kenya", "Pakistan", "Switzerland", "France")) # List does not include DRC or Burkina Faso b/c opensource data is not available thorugh Nextstrain.  This list doesn't include all the african countries or the WHO countries. 
...
df10 <- df9 %>% dplyr::filter(site_id %in% c(1:13, 15:21,2201,2202,23:38)) %>% # Removed #14 (Africa for now 20220919)
  ...
  dplyr::select(site_id, city, country, site_name, person_id, twin, triplet, twin2, tripletnum, 
...
temp.df <- df10 %>% dplyr::select(country, site_name, date_onset_or_test) %>%
  filter(country==c("USA", "Kenya"))
...
df10.2 <- df10.1 %>% dplyr::select(site_id, city, country, site_name, date_onset_or_test, date_ym) 
...
temp.df <- df10.3 %>% dplyr::filter(country=="Turkey")
```
...
sf_fig_df <- sf_fig_df %>% mutate(site_name = country,
                                  study_type = "RFA",
                                  study_type_full = "Risk Factor Analysis")
...
ggsave(filename = 'data_out/covid_variant_country_20221012.pdf', width = 6, height = 20, units = "in") #This includes Colombia data (which was excluded b/c i had it spelled ColUmbia before)
...
df11m.1 <- df11m %>% group_by(site_name, country, dominant_variant, outcome, date_ym) %>%
...
df11m.3_wide <- df11m.2_wide %>% mutate(Total = sum(events0, events1)) %>%
  dplyr::select(-events0)
```

***ADD IN SWISS/FRANCE DATA HERE***
```{r}
sf_df4 <- left_join(sf_df3, df8_wide, by=c('country', 'date_ym', 'dominant_variant'))

sf_df5 <- sf_df4 %>% dplyr::select(site_name, outcome, country, date_ym, dominant_variant, Events, Missing, Total)
temp.df <- compiled_ipd_data %>% dplyr::select( preterm_labor0, preterm_labor1, preterm_laborm,denom_endpreg, site_id)
```


```{r}
sf_df3.1 <- sf_df3 %>% rename(events1 = Events, eventsm = Missing)
merged_df <- rbind(df11m.3_wide, sf_df3.1)
```
...
df14 <- df13 %>% group_by(site_name, country, dominant_variant, outcome) %>%
  summarize(events = sum(events, na.rm = FALSE),
            tot_n = n(),
            incidence = events/tot_n)
temp.df <- df14 %>% dplyr::filter(country=="USA")
...
temp.df <- df14 %>% dplyr::filter(country=="Turkey")
...
temp.df <- df15 %>% dplyr::filter(country=="USA")
```
...
```{r, include=FALSE}
df14.negpos.outcome <- df15 %>% mutate(neg = tot_n - events)
...
# http://127.0.0.1:57661/help/library/meta/help/meta-object
  ...
  pdf(file=paste0("plots/20221005_domvar_mixed_>=60/VariantStudy_MetaForestPlot_ref_", refstr, "_test_", teststr, ".pdf"), width=10, height=65) # pdf saving has to go before making the plot and at the end have to say dev.off()
...
outcome_vec <- c("matdeath","covid_hosp1", "ICUadmit1", "covid_icu", "critcare1", "ventilation1", "pneumonia1",
                "rdeath1", "place_abrupt1", "preterm1", "preterm_labor", "haemorrhage1", "embolicdz1",
                "preeclampsia1", "eclampsia1", "pre_or_eclampsia1", "hpd_any1", "hpd_postcovid1", "c_section1",
                "c_intrap1","mat_composite_outcome",
                # fetal outcomes: 
                "stillbirth_site1", "stillbirth_281", "sab_site1", "tab_site1", "neonatal_death1", "earlyneo_death1", 
                "perinatal_death281", "perinatal_deathsite1", "nicu1","lowbirthweight1", "verylowbirthweight1",
                 "verypreterm1",   "sga1", "extremesga1", 
                "fetal_composite_outcome")
...
for (rs in levels(factor(heatmap_data_df2$Ref_Strain))) {
  ...
  heatmap_data$RR_sig <- factor(heatmap_data$RR_sig, 
                                 levels=c("RR>1, CI-sig", "RR<1, CI-sig", 
                                          "RR>1, CI-notsig", "RR<1, CI-notsig"),
                                ordered = TRUE)
...
heatmap_data$Outcome <- factor(heatmap_data$Outcome, 
  ...
  colScale <- c("RR>1, CI-sig" = "orangered", 
                "RR<1, CI-sig" = "dodgerblue", 
                "RR>1, CI-notsig"= "thistle2", 
                "RR<1, CI-notsig" = "lightcyan") 
 ...
 hmap <- heatmap_data %>% dplyr::filter(n_study>1) %>%
 ...
 ggsave(paste0('plots/heatmap/dom_strain>=50/variant_study_heatmap_',
               format(Sys.time(), "%Y-%m-%d_%H-%M"),
               "_refstrain", 
               rs,
               "_n_study>1.pdf"),
...
ggsave(filename = 'data_out/covid_variant_country_heatmpa_20220919.pdf', width = 10, height = 6, units = "in")
```
