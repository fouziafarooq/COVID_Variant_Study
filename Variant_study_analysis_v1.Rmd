...
ggsave(filename = 'plots/variants_nextstrain/covid_variant_country_20221101.pdf', width = 6, height = 20, units = "in") #This includes Colombia data (which was excluded b/c i had it spelled ColUmbia before)
...
df8.1 <- df8 %>% group_by(country, date_ym) %>%
  ...
  mutate(dominant_variant = if_else(first(freq)==0,
                                    "no_strain", 
                                    # first(variant))) %>% 
                                      ifelse(!is.na(first(nor)) & first(nor)>=0.50, # This is the other way of defining the dominant variant-- when it's greater than 60% then call it the dominant variant, otherwise call the set mixed. 
                                           first(variant), 
                                           "mixed"))) %>% 
...
***#TODO: Issue is here.  There is no eventsm.  I need to create Total in the IPD data here.***
...
sf_df5 <- sf_df4 %>% dplyr::select(site_name,  country, outcome, date_ym, dominant_variant, dominant_variant_v1, events1, eventsm, Total)
```
...
*#TODO make sure the above chunks are correct before proceeding.*
*#TODO Remove outcomes that are missing more than 25% in total in each country* No need to do this by variant.
...
```{r, include=FALSE}
df14.negpos.outcome <- merged_df2 %>% mutate(neg = Total - events1)
...
# http://127.0.0.1:57661/help/library/meta/help/meta-object
  ...
  pdf(file=paste0("plots/20221103_domvar_mixed/VariantStudy_MetaForestPlot_ref_", refstr, "_test_", teststr, ".pdf"), width=10, height=65) # pdf saving has to go before making the plot and at the end have to say dev.off()
