...
df4 <- df3 %>% group_by(country, week) %>% 
  ...
  dplyr::filter(country %in% c("Kenya",  "Spain", "Hong Kong", "Italy",
                               ...
                               "USA", "India", "Kenya", "Pakistan")) # List does not include DRC or Burkina Faso b/c opensource data is not available thorugh Nextstrain.  This list doesn't include all the african countries or the WHO countries. 
...
ggsave(filename = 'data_out/covid_variant_country_20220919.pdf', width = 6, height = 30, units = "in") #This includes Colombia data (which was excluded b/c i had it spelled ColUmbia before)
...
temp.df <- df13 %>% dplyr::filter(site_id==17)
```
