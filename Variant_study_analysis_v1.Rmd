...
sort(colnames(compiled_ipd_data_new))
```
...
df10 <- df9 %>% dplyr::filter(site_id %in% c(1:21,2201,2202,23:38)) %>% 
  ...
  dplyr::select(site_id, city, country, site_name, mat_age, date_onset_or_test, covid_sympdate, covid_pos,
                edd_date, fetuses, parity, gravidity, weight_pre, matdeath,
                matdeath_date, covid_hosp, covid_icu, preeclampsia, eclampsia, pre_or_eclampsia,
                place_abrupt, preterm_labor, haemorrhage, hpd_any, c_section1, 
                stillbirth_site1, stillbirth_281, nicu1, lowbirthweight1, verylowbirthweight1, 
                sab_site1, c_intrap1, verypreterm1) 
# csection1 = All C-Sections (among all stillbirths + livebirths)
...
df12 <- df11 %>% pivot_longer(cols = c(matdeath, covid_hosp, covid_icu, preeclampsia, eclampsia, pre_or_eclampsia,
                place_abrupt, preterm_labor, haemorrhage, hpd_any, c_section1, 
                stillbirth_site1, stillbirth_281, nicu1, lowbirthweight1, verylowbirthweight1, 
                sab_site1, c_intrap1, verypreterm1),
...
# manual process in STATA - replaced 0 with the inverse of the opposite arm. can try this code. Making the continuity correction smaller helps but not always. Pooled absolute risks and pooled relative risks that were illogical, absolute risk was high for one group but relative risk was lower for that group.   If there are 0 events for delta and alpha, what to do with that estimate. 
  ...
  pdf(file=paste0("plots/20220810_domvar_mixed_70/VariantStudy_MetaForestPlot_", refstr, "_", teststr, ".pdf"), width=10, height=22) # pdf saving has to go before making the plot and at the end have to say dev.off()
