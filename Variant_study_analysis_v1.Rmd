...
df10 <- df9 %>% dplyr::filter(site_id==c(4,5,17,10,15,19,20,24,34,38)) %>% 
...
Summarize data
```{r}
table(df11$preterm_labor)
table(df11$ diab_prepreg)
df12 <- df11 %>% group_by(country, dominant_variant) %>% summarize(count_condition=sum(diab_prepreg==1),
                                                           n=n())
```
...
agd_results2 <- agd_df3.2 %>% group_by(country, variant_mod, Outcome) %>% summarize(count_condition=sum(Events),
                                                                       tot_n = sum(Total),
                                                                       incidence = count_condition/tot_n) 
