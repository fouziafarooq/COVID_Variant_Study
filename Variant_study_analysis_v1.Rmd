...
Dates are fractions of a year after "year." We a creating a separete dataframe into a csv that we will bring in R and use lubridate to read the date properly. 
...
# Newesst data with mistakes fixed sent 12/08/2022
...
df4 <- df3 %>% group_by(country, week) %>% 
  ...
  dplyr::filter(country %in% c("Kenya", "Spain", "Hong Kong", "Italy",
                                "USA", "Mexico", "Switzerland", "France")) # List does not include DRC or Burkina Faso b/c opensource data is not available thorugh Nextstrain.  This list doesn't include all the african countries or the WHO countries. 
...
df10 <- df9 %>% dplyr::filter(site_id %in% c(1:13, 15:21,2201,2202,23:35,37)) %>% # Removed #14 (Africa for now 20220919)
...
# http://127.0.0.1:57661/help/library/meta/help/meta-object
  ...
  dir.create(paste0("plots/",dir_folder_name), recursvie = TRUE)
...
dir.create(paste0("data_out/pooled_tables/",dir_folder_name), recursvie = TRUE)
...
heatmap_data$Outcome <- factor(heatmap_data$Outcome, 
 ...
 dir_folder_name <- paste0(day_string,"_dom_strain_",as.character(dom_threshold*100))
  dir.create(paste0("plots/heatmap/",dir_folder_name), recursvie = TRUE)
