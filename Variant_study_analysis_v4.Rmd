...
gisaid_df <- read_delim('data/global.tsv',
                 delim = '\t') %>%
...
compiled_ipd <- read_dta('data/Compiled_IPD_Data_240319.dta') # Most updated
...
# NOTE: Erin gave me below files in the compiled_idp_20240319 file. 
```{r}
...
###############################################################
###############################################################
***STOPPED HERE - Not sure what to do next as of 01/09/2024***
###############################################################
###############################################################
###############################################################
...
gisaid_df2 <- gisaid_df2 %>%
  filter(country %in% c("Spain", "Mexico", "USA", "Italy", "Kenya", "Hong Kong", "Japan", 
...
```{r}
  ...
  rr_ff <- function(pos_test, pos_ref, neg_test, neg_ref, adj=0.5){
    ...
    a <- pos_test+useadj*adj #TODO: This will be 0.
