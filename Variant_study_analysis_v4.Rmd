...
output: pdf_document
---
# This is the working copy as of 03/15/2024
...
########################################################################
NOTES:
# 2024-05-06: PMA_Analysis_Variables_QS2_Toulouse.dta - use this data file to add in France (site 55 to the dataset). NOTE: I do need to remove site 55 from the 'Compiled_IPD_Data_240319.dta' first b/c it has an older version of the data.
########################################################################
...
#If redownload this data make sure you rename it with the right date.  I downloaded this for the Variants - case comparison.Rmd work also on 20241126.
if (redownload) {
...
# Study files # 
################
compiled_ipd <- read_dta('data/Compiled_IPD_Data_240319.dta') # Most updated from Erin

compiled_ipd <- compiled_ipd %>% filter(site_id!=55)

# Read in the France file (Site 55) and add it onto compiled_ipd.
france_55 <- read_dta('data/PMA_Analysis_Variables_QS2_Toulouse.dta')
jozwiak_49 <- read_dta('data/PMA_Analysis_Variables_QS2_COVIDPREG.dta') # I see in the notes that this is data only from France. Belgium and Switzerland were dropped due to small sample size. 
sort(colnames(jozwiak_49))
str(france_55$hpd_date)
france_55$hpd_date <- as.Date(france_55$hpd_date)
compiled_ipd$neo_death_date3 <- as.Date(compiled_ipd$neo_death_date3)

compiled_ipd <- bind_rows(compiled_ipd, france_55)

sort(colnames(compiled_ipd))
compiled_ipd$date_onset_or_test
...
country_id <- read.csv('data/country_id_ipd_3_20240320.csv') # FF created this file. THIS HAS ONLY 1 PAKISTAN SITE (FROM AMHANII)

```

# Summary Info for ROB tables - 2024
```{r}
temp.df <- compiled_ipd %>%
  filter(site_id=="68") 
sum(!is.na(temp.df$covid_testdate))

results <- compiled_ipd %>%
  filter(twin2==0) %>%
  filter(triplet==0) %>%
  group_by(site_id) %>%
  summarize(
    total_rows = n(),
    missing_covid_sympdate = sum(is.na(covid_sympdate)),
    non_missing_covid_sympdate = sum(!is.na(covid_sympdate)),
    percent_non_missing_covid_sympdate = sum(!is.na(covid_sympdate)) / n() * 100,
    missing_covid_testdate = sum(is.na(covid_testdate)),
    non_missing_covid_testdate = sum(!is.na(covid_testdate)),
    percent_non_missing_covid_testdate = sum(!is.na(covid_testdate)) / n() * 100
  )

```

Write out the IPD file for Erin - for Table 1. 
```{r}
# write.csv(compiled_ipd, 'data_out/compiled_ipd_20240509.csv')
# sort(colnames(compiled_ipd))
```


# NOTE: Erin gave me below files in the compiled_idp_20240319 file upstairs!. 
```{r}
...
```{r}
# sort(colnames(compiled_ipd))
temp.df <- compiled_ipd  %>% select(site_id)
```

FIX DATE TYPE: 
```{r}
# FIRST CREATE A VECTOR OF ALL THE STUDIES BEING ADDED:
# df_vec <- list(compiled_ipd, texas_saad_df, ny_rommel_df, france_jozwiak_df, cdc_espi_df, 
...
compiled_ipd$hemoglobin_date <- as.Date(compiled_ipd$hemoglobin_date, origin = "1960-01-01")
compiled_ipd$vacc_covid1_date <- as.Date(compiled_ipd$vacc_covid1_date, origin = "1960-01-01")
compiled_ipd$vacc_covid2_date <- as.Date(compiled_ipd$vacc_covid2_date, origin = "1960-01-01")
compiled_ipd$vacc_booster_date <- as.Date(compiled_ipd$vacc_booster_date, origin = "1960-01-01")
compiled_ipd$birth_date <- as.Date(compiled_ipd$birth_date, origin = "1960-01-01")
str(compiled_ipd$hemoglobin_date)
...
compiled_ipd <- fix_dates(compiled_ipd)
...
```{r}
# df <- bind_rows(compiled_ipd, texas_saad_df)
...
AGGREGATE DATA FROM SWITZERLAND &FRANCE (GUILLAUME); JAPAN (SONJI); Philly (Flannery); US (Neelam)
```{r}
...
# JAPAN - SONJI
################
japan_sheets_vec <- excel_sheets('/Users/fouziafarooq/Library/CloudStorage/Box-Box/Perinatal COVID-19 PMA/Data for Analysis/Round 2 Analysis - January 2021/72. Japan (Shoji)/Variant_Study - Shell Table_Aggregate Data Request_Japan_FF_editsForR.xlsx') # creates a vector, SITE # 72
japan_shoji_df <- data.frame(month={}, year={}, Events = {}, Missing={}, Total={})  
for (sheet in japan_sheets_vec) {
  ...
  sheet_df <- sheet_df %>% dplyr::mutate(Outcome=sub("Japan-", "", sheet),
                                         country = "Japan",
                                         author = "Shoji",
                                         author_year = "Shoji, 2023")
  japan_shoji_df <- rbind(japan_shoji_df, sheet_df)
}

# Rename variable names
japan_shoji_df <- japan_shoji_df %>% mutate(month_year = paste(month_infection, year_infection)) %>%
  mutate(month_year = gsub(" ","", month_year))

# Function to remove "20" from each element of the column
remove_20 <- function(month_year) {
  return(sub("20", "", month_year, fixed = TRUE))
}

# Apply the function to the entire column
japan_shoji_df <- japan_shoji_df %>%
  mutate(month_year = remove_20(month_year)) %>% 
  select(-month_infection, -year_infection)

####################
# PHILLY - FLANNERY
####################
philly_flannery_vec <- excel_sheets('/Users/fouziafarooq/Library/CloudStorage/Box-Box/Perinatal COVID-19 PMA/Data for Analysis/Round 2 Analysis - January 2021/71. Philly (Flannery)/COVID-19 Variant Study_5_31_23_AZB_FF_EditsForR.xlsx') # SITE 71

philly_flannery_df <- data.frame(Month={}, Year={}, events={},	missing={},	total={})
for (sheet in philly_flannery_vec) {
  sheet_df <- read_excel('/Users/fouziafarooq/Library/CloudStorage/Box-Box/Perinatal COVID-19 PMA/Data for Analysis/Round 2 Analysis - January 2021/71. Philly (Flannery)/COVID-19 Variant Study_5_31_23_AZB_FF_EditsForR.xlsx', 
                         sheet=sheet, 
                         skip=2, # skips the empty lines.
                         col_names=c("Month", "Year", "events", "missing", "total"))
  sheet_df <- sheet_df %>% dplyr::mutate(Outcome= sheet,
                                         country="USA",
                                         author = "Flannery",
                                         author_year = "Flannery, 2022")
  philly_flannery_df <- rbind(philly_flannery_df, sheet_df)
}

# concatenate month/year:
philly_flannery_df <- philly_flannery_df %>% mutate(month_year = paste(Month, Year)) %>%
  mutate(month_year = gsub(" ","", month_year)) %>%
  select(-Month, - Year) %>%
...
# Apply the function to the entire column
philly_flannery_df <- philly_flannery_df %>%
  mutate(month_year = remove_20(month_year)) 

############################
# USA - CDC-SETNET - NEELAM - 
# NOTE 20240823: This is study # 73 but was listed in the EPPI reviewer sheet as 54.  
###### I have updated this in the country_id_ipd_3_20240320.csv file to #73.
############################
# maternal data
usa_neelam_mat_sheet_vec <- excel_sheets('/Users/fouziafarooq/Library/CloudStorage/Box-Box/Perinatal COVID-19 PMA/Data for Analysis/Round 2 Analysis - January 2021/73. CDC SETNET/output/PMA_variant_SiteEstimates_Maternal_CDC_SETNET_FF.xlsx') # MATERANAL OUTPUT FILE.  SITE 73

usa_neelam_mat_df <- data.frame(month_year={}, Events={},	Total={}, proportion={}, lowerCI={}, upperCI={}, missing_outcome_data={})
for (sheet in usa_neelam_mat_sheet_vec) {
  sheet_df <- read_excel('/Users/fouziafarooq/Library/CloudStorage/Box-Box/Perinatal COVID-19 PMA/Data for Analysis/Round 2 Analysis - January 2021/73. CDC SETNET/output/PMA_variant_SiteEstimates_Maternal_CDC_SETNET_FF.xlsx', 
                         sheet=sheet, 
                         skip=2, # skips the empty lines.
                         col_names=c("Month_Year", "Events", "Total", "Proportion", "Lower CI", "Upper CI","Missing Outcome Data"))
  sheet_df <- sheet_df %>% dplyr::mutate(Outcome=sheet,
                                         country="USA",
                                         author = "Neelam",
                                         author_year = "Neelam, 2022")
  usa_neelam_mat_df <- rbind(usa_neelam_mat_df , sheet_df)
}

# Infant data
usa_neelam_infant_sheet_vec <- excel_sheets('/Users/fouziafarooq/Library/CloudStorage/Box-Box/Perinatal COVID-19 PMA/Data for Analysis/Round 2 Analysis - January 2021/73. CDC SETNET/output/PMA_variant_SiteEstimates_Infant_CDC_SETNET_FF.xlsx') # INFANT OUTPUT FILE. SITE 73

usa_neelam_infant_df <- data.frame(month_year={}, Events={},	Total={}, proportion={}, lowerCI={}, upperCI={}, missing_outcome_data={})
for (sheet in usa_neelam_infant_sheet_vec) {
  sheet_df <- read_excel('/Users/fouziafarooq/Library/CloudStorage/Box-Box/Perinatal COVID-19 PMA/Data for Analysis/Round 2 Analysis - January 2021/73. CDC SETNET/output/PMA_variant_SiteEstimates_Infant_CDC_SETNET_FF.xlsx', 
                         sheet=sheet, 
                         skip=2, # skips the empty lines.
                         col_names=c("Month_Year", "Events", "Total", "Proportion", "Lower CI", "Upper CI","Missing Outcome Data"))
  sheet_df <- sheet_df %>% dplyr::mutate(Outcome=sheet,
                                         ...
                                         author = "Neelam",
                                         author_year = "Neelam, 2022")
  usa_neelam_infant_df <- rbind(usa_neelam_infant_df , sheet_df)
}

# Stack maternal and infant data for Neelam CDC SETNET study.
usa_neelam_df <- rbind(usa_neelam_mat_df, usa_neelam_infant_df)

# rename variables and remove columns that are not needed: 
usa_neelam_df <- usa_neelam_df %>% rename(month_year = Month_Year,
                                    Missing = `Missing Outcome Data`) %>% 
  select(-Proportion, -`Lower CI`, -`Upper CI`)

usa_neelam_df <- usa_neelam_df %>% 
  mutate(month_year = as.Date(month_year, format = "%y-%m-%d" ), 
                             month_year = format(month_year, "%B%y"))
```

# For ROB - computing missingness
```{r}
##########
# France #
##########
france_missingness <- france_df %>%
  group_by(Outcome) %>%
  summarize(missing_pct = round(100*sum(Missing)/sum(Total), digits = 0))

###############
# Switzerland #
###############
swiss_missingness <- swiss_df %>%
  group_by(Outcome) %>%
  summarize(missing_pct = round(100*sum(Missing)/sum(Total), digits = 0))
...
# Japan #
###############
japan_missingness <- japan_shoji_df %>%
  group_by(Outcome) %>% 
...
# Philly - Flannery #
#####################
flannery_missingness <- philly_flannery_df %>%
  group_by(Outcome) %>% 
  filter(!is.na(Missing)) %>%
  summarize(missing_pct = round(100*sum(Missing)/sum(Total), digits = 0))
...
swiss_france_df <- swiss_france_df %>%
  ...
  mutate(Outcome = sub('switzerland - ', "", Outcome))
table(swiss_france_df$Outcome)
```
...
rbind other aggregate data sites (Japan (Shoji), Philly (Flannery), CDC-SETNET (Neelam))
```{r}
japan_philly_setnet_df <- rbind(japan_shoji_df, philly_flannery_df, usa_neelam_df)
japan_philly_setnet_df$month_year <- str_to_title(japan_philly_setnet_df$month_year)
japan_philly_setnet_df <- japan_philly_setnet_df %>%
  mutate(month_year = format(as.Date(paste0("01-", month_year), format = "%d-%B%y"), "%y-%m")) %>% # "%d-%B%y" is the format of the input string.
  rename(date_ym = month_year)

```

Now rbind swiss_france_df with japan_philly_setnet_df:
```{r}
agg_df <- rbind(swiss_france_df, japan_philly_setnet_df)
```

REMOVING  PAKISTAN (AMHANII), BRAZIL (REBRACO) FROM THE DATA:THE OVERLAP PAST PreALPHA for rebraco IS NOT ENOUGH.
```{r}
compiled_ipd <- compiled_ipd %>% dplyr::filter(site_id !=38)
compiled_ipd <- compiled_ipd %>% dplyr::filter(site_id !=36)
compiled_ipd <- compiled_ipd %>% dplyr::filter(site_id !=35) # Padua, Italy (doesn't have overlapping data)
```

```{r}
table(compiled_ipd$site_id)
```


```{r, include=FALSE}
site_id_new <- compiled_ipd$site_id
compiled_ipd_data <- compiled_ipd
```
...
# For ROB tables
temp.df <- compiled_ipd %>%
  select(site_id, covid_sympdate, covid_testdate, cov_type_test) %>%
  filter(site_id == 74) 
...
results2 <- left_join(results, country_id, by = "site_id")
results2 <- results2 %>%
  relocate(site_id, country, site_name, author_year, .before=everything())
...
***ADD IN aggregate data from Swiss, France, Japan, Philly, and CDC-SETNET***
```{r}
########## NOTE #############################################################################
# preterm2 and verypreterm2 (for sensitivity analysis) seem to be only in SWISS/France data.
#############################################################################################
agg_df2<- agg_df %>% rename(outcome=Outcome, 
                            events1=Events,
                            eventsm=Missing) %>%
...
ADD IN -01 TO THE DATE VAR:
```{r}
agg_df2 <- agg_df2 %>% dplyr::filter(!is.na(date_ym))

agg_df2 <- agg_df2 %>% mutate(date = as.Date(paste0("20", date_ym, "-01")))

agg_fig_df <- agg_df2 %>% group_by(country, date, author_year) %>% 
  summarize(Events = sum(events1))

agg_fig_df <- agg_fig_df %>% mutate(pandemic_month = interval("2019-12-01", date) %/% months(1))
```

ADD IN -01 TO THE DATE.
```{r}
# agg_df2 <- sf_df %>% dplyr::filter(!is.na(date_ym))
...
```{r}
# 5 = Madrid, Spain (Gil, Fernandez Buhigas, 2021)
# 10 = Mexico (Martinez-Portilla, 2021)
# 15 = Kenya (Akelo, Tippett Barr, 2021)
# 17 = Puertro Rico (Bracero, Valencia, Delgado-Lopez, 2021)
# 19 = Rome, Italy (Bevilacqua, Laurita Longo, 2020)
# 20 = Hong Kong (Poon, 2021)
# 35 = Padua, Italy
# 37 = Brazil (WHO)
# 39 = Pakistan (WHO)
# 44 = Texas (Saad)
...
# 59 = 
# 60 = 
# 62 = Madrid (Casado, 2022)
# 66 = 
# 68 = 
# 72 = Japan (Shoji, 2023)
# 74 = 
gisaid_df2 <- gisaid_df2 %>%
  filter(country %in% c("Spain", "USA",  "Kenya", "Hong Kong", "Italy", "Brazil", "Japan", 
                          "Malaysia", "Turkey", "Saudi Arabia", "India", "Switzerland", "France", "Philippines", "Pakistan")) %>% 
  # Mexico dropped out in 2024 analysis run b/c didn't hav overlapping data.
...
### THRESHOLD SETTING ###
#########################
dom_threshold <- 0.50 # This is 50% prevalence/dominance
...
```
UPDATE Gajbhiye DATES - based on email communication on 12/05/2024
```{r}
compiled_ipd_data <- compiled_ipd_data %>% 
  mutate(date_onset_or_test = if_else((site_id==66 & date_onset_or_test=="2022-12-29"), ymd("2022-01-29"), date_onset_or_test),
         date_onset_or_test = if_else((site_id==66 & date_onset_or_test=="2022-11-09"), ymd("2022-01-09"), date_onset_or_test))

temp.df2 <- compiled_ipd_data %>%
  filter(site_id==66) %>% 
  select(site_id, SUBJID, date_onset_or_test) 
...
df10 <- df9 %>% dplyr::filter(site_id %in% c(1:9, 11:13, 15:21,2201,2202,23:35,37, 39, 44:46, 48:60, 
                                             62, 66, 68, 71, 72, 74)) %>% # Removed #14 (Africa for now 20220919) and Mexico (#10)
...
temp.df <- compiled_ipd_data %>% select(site_id)
...
```{r}
df10.8_agg_df <- rbind(df10.8, agg_fig_df)

colnames(df10.8_agg_df)
temp.df <- df10.8_agg_df %>% select('country')
```
...
studies_df <- df10.8_agg_df %>% dplyr::filter(Events>=1) %>% 
...
df_monthly$variant <- fct_relevel(df_monthly$variant,  
                                  "Pre-alpha", "Alpha", "Beta", "Gamma",  "Delta", "Epsilon", "Eta", "Theta", "Iota", "Lambda", "Mu", "Omicron") # Kappa, recombinant
...
***ADD IN Aggregate data here***
```{r}
sf_df4 <- left_join(agg_df2, df8_wide, by=c('country', 'date_ym'))
...
merged_df3 %>% 
 # filter(dominant_variant!="Beta") %>% # beta numbers are very low. 
  ...
  geom_col(aes(x=dominant_variant, y=incidence, fill=outcome), 
           width = 0.65,
           position="dodge") + 
  scale_x_discrete(limits = c("no_strain", 'mixed', "Pre-alpha", "Alpha", "Beta", "Gamma", "Delta", "Omicron")) + # "XBB",
...
# http://127.0.0.1:57661/help/library/meta/help/meta-object
  ...
  pdf(file=paste0("plots/",dir_folder_name,"/VariantStudy_MetaForestPlot_teststr_", teststr, "_refstr_", refstr, ".pdf"), width=10, height=150) # pdf saving has to go before making the plot and at the end have to say dev.off()
...
heatmap_data_df2 <- heatmap_data_df %>% 
  mutate(RR_CI_lab = paste0(format(round(RR, 2), nsmall=2),
                            " (", 
...
est_table_df2 <- est_table_df %>%
  mutate(Ref_Strain = factor(Ref_Strain, 
                             levels = c("Pre-alpha", "Alpha" , "Gamma", "Delta", "Omicron","recombinant", "mixed"), # "Beta", "Epsilon", "Eta", "XBB", "recombinant",
                             labels = c("Pre-alpha", "Alpha", "Gamma",  "Delta", "Omicron", "recombinant", "mixed"))) %>% # "Beta", "Eta", "Epsilon", "XBB", "recombinant",
...
###############!!!!!!!!!!!!!!!!!!!!!!!!!#####################
  ...
  select(Outcome, `Reference Strain` = Ref_Strain, `Pre-alpha` = Pre-alpha, Alpha, Gamma, Delta, Omicron, Mixed = mixed) # Beta, Epsilon, Eta, Eta, Recombinant = recombinant
...
heatmap_data$Outcome <- factor(heatmap_data$Outcome, 
  ...
  mutate(Strain = factor(Strain, 
                         levels = c("Pre-alpha", "Alpha", "Beta", "Gamma", "Epsilon", "Eta", "Delta",  "Omicron", "mixed"), # "XBB", "Recombinant", "Epsilon", "Eta",
                         labels = c("Pre-alpha", "Alpha", "Beta", "Gamma", "Epsilon", "Eta", "Delta", "Omicron", "mixed"))) %>% # "XBB", "Recombinant",
    mutate(Outcome = recode_factor(Outcome, 
                                 # Adverse Birth outcomes
                                 "verylowbirthweight" = "vLBW", 
                                 "lowbirthweight" = "LBW", 
  ...
  hmap <- heatmap_data2 %>% 
    dplyr::filter(n_study>=1) %>%
    filter(!(Strain %in% c("Epsilon", "Eta"))) %>%
...
heatmap_long2 <- heatmap_long2 %>% 
  ...
  mutate(Strain = factor(Strain, levels = c("Prealpha", "Delta", "Epsilon", "Omicron", "recombinant", "Mixed"), ordered = TRUE)) %>% #  "Recombinant", 
